...
import type { DimensionId, GaugeConfig } from "@/lib/types";

export function generateStaticParams() {
  return getAllGaugeIds().map((slug) => ({ slug }));
}

function dimensionNamesFor(config: GaugeConfig): string {
  ...
  return ids
    .map((id) => {
      const name = gaugesConfig.dimensions.find((d) => d.id === id)?.name ?? id;
...
export default async function GaugeDetailPage({
  ...
  const dimensionNames = dimensionNamesFor(config);
  const isUnscored = (config.unscoredDimensions?.length ?? 0) > 0;

  if (isUnscored) {
    return <UnscoredGaugeDetail config={config} data={data} dimensionNames={dimensionNames} slug={slug} />;
  }
...
/**
 * Dedicated page shape for a gauge with `unscoredDimensions` set — real
 * data (when present) shown as a plain time series, deliberately with no
 * level score, no DirectionArrow, no dot strip, no rank chart, since none
 * of those are meaningful for data that was never fed into a composite.
 * The reason is shown first and prominently, never buried below a number
 * that doesn't exist — see cohesion-majority-acceptance's config for the
 * first real case this was built for.
 */
function UnscoredGaugeDetail({
  config,
  data,
  dimensionNames,
  slug,
  ...
  config: GaugeConfig;
  data: ReturnType<typeof getGaugeData>;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      {config.unscoredReason && (
        <div className="mt-4 rounded-lg border border-[var(--gridline)] bg-[var(--surface-1)] p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--text-muted)]">
            Why this gauge isn&rsquo;t scored
          </p>
          <p className="mt-2 text-[var(--text-secondary)]">{config.unscoredReason}</p>
        </div>
      )}
      ...
      {data ? (
        <section className="mt-8">
          <h2 className="mb-3 text-lg font-semibold">Every country&rsquo;s published values</h2>
          <p className="mb-3 text-sm text-[var(--text-secondary)]">
            Shown as raw values over time — not as a peer-relative score, dot strip, or rank, since
            those would imply a comparison this data can&rsquo;t honestly support.
          </p>
          <TimeSeriesChart data={data} />
        </section>
      ) : (
        <div className="mt-6 rounded-lg border border-[var(--gridline)] bg-[var(--surface-1)] p-6">
          ...
          <p className="mt-2 text-[var(--text-secondary)]">
            It won&rsquo;t be scored once entered either way — see &ldquo;Why this gauge isn&rsquo;t
            scored&rdquo; above.
          </p>
        </div>
      )}
