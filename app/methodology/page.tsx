...
/** REGISTER link treatment (DESIGN.md: "Links are --ink with a --chrome underline that darkens to --ink on hover — no blue anywhere"). */
const LINK = "underline decoration-chrome hover:decoration-ink";
/** Every numeral, ISO code, dataflow ID, and filename on this page is apparatus content — Martian Mono (DESIGN.md Typography), not the browser-default monospace the pre-REGISTER `<code>` tags rendered in. */
const CODE = "font-martian-mono text-[0.92em]";
  ...
  return (
    <div className="register bg-paper font-public-sans text-ink min-h-screen p-6 sm:p-[48px_56px_56px]">
      <div className="mx-auto max-w-4xl">
        <h1 className="font-bold text-[28px] sm:text-[36px] tracking-[-.01em]">Methodology</h1>
        <p className="mt-3 max-w-2xl text-ink-2">
          This page is generated directly from{" "}
          <code className={CODE}>gauges.config.json</code>
        ...
        {/* Primary separator under the page header — 3px --ink, same weight
            DESIGN.md assigns the rule under a gauge page's own header. */}
        ...
        <section className="mt-10 pt-8 border-t border-ink">
        ...
        <section className="mt-10 pt-8 border-t border-ink">
          ...
          <p className="mt-3 text-ink-2">
            ...
            <Link href="/gauges/housing-pressure" className={LINK}>
              Housing pressure
            </Link>
        ...
        <section className="mt-10 pt-8 border-t border-ink">
          ...
          <div className="mt-3 space-y-2">
            {gaugesConfig.dimensions
              .find((d) => d.id === "power")!
              .scoreBands.map((b) => (
                <div key={b.id} className="flex items-center gap-3 text-[13.5px]" aria-label={`Band: ${b.label}`}>
                  <span
                    className="font-martian-mono text-[11px] font-semibold tracking-[.06em] text-ink-3 w-14 shrink-0"
                    aria-hidden="true"
                  >
                    {b.ticks}
                  </span>
                  <span className="w-36 shrink-0 font-medium">{b.label}</span>
                  <span className="font-martian-mono tabular-nums text-ink-2">
                    {b.min}&ndash;{b.max}
                  </span>
                </div>
              ))}
        ...
        <section className="mt-10 pt-8 border-t border-ink">
          <h2 className="text-xl font-semibold mb-4">Every gauge, in full</h2>
          {gaugesConfig.dimensions.map((dimension) => (
            <div key={dimension.id} className="mb-8 last:mb-0">
              ...
              {gaugesConfig.gauges
                ...
                .map((g, i) => {
                  const notScored = g.weights[dimension.id] === undefined;
                  return (
                    <div key={g.id} className={i === 0 ? "pt-4" : "pt-4 mt-4 border-t border-grid"}>
                      <div className="flex flex-wrap items-baseline justify-between gap-2">
                        <h4 className="text-lg font-semibold">{g.name}</h4>
                        <span className={`font-martian-mono text-[11px] tracking-[.04em] ${notScored ? "text-stamp font-bold" : "text-ink-2"}`}>
                          {notScored
                            ? "NOT SCORED"
                            : `WEIGHT IN ${dimension.shortName.toUpperCase()}: ${((g.weights[dimension.id] ?? 0) * 100).toFixed(1)}%`}
                        </span>
                      </div>
                      {g.unscoredReason && (
                        <p className="mt-2 border border-chrome text-[13px] leading-[1.6] text-ink-2 p-2.5">
                          {g.unscoredReason}
                        </p>
                      )}
                      {Object.keys(g.weights).length > 1 && (
                        <p className="mt-1 font-martian-mono text-[10.5px] text-ink-3">
                          Also scored in{" "}
                          {gaugesConfig.dimensions
                      ...
                      <dl className="mt-3 grid grid-cols-1 gap-x-6 gap-y-2.5 text-[13.5px] sm:grid-cols-2">
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">UNIT</dt>
                          <dd>{g.unit}</dd>
                        </div>
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">SOURCE INSTITUTION</dt>
                          <dd>
                            <a href={g.source.url} className={LINK} target="_blank" rel="noreferrer">
                              {g.source.institution}
                            </a>
                          </dd>
                        </div>
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">SERIES ID</dt>
                          <dd>
                            <code className={CODE}>{g.source.seriesId}</code> &mdash; {g.source.seriesName}
                          </dd>
                        </div>
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">ACCESS TYPE</dt>
                          <dd className="capitalize">{g.accessType}</dd>
                        </div>
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">POLARITY</dt>
                          <dd className="capitalize">{g.polarity.replace(/_/g, " ")}</dd>
                        </div>
                        <div>
                          <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">HISTORY TARGET</dt>
                          <dd>{g.historyStartYear} &ndash; present</dd>
                        </div>
                        {g.evidenceStrength === "survey" && (
                          <div>
                            <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">EVIDENCE STRENGTH</dt>
                            <dd>Survey-based (self-report), not a directly measured hard statistic</dd>
                          </div>
                        )}
                        {g.scoringBasis === "latest-wave-per-country" && (
                          <div>
                            <dt className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">SCORING BASIS</dt>
                            <dd>Latest-wave-per-country &mdash; see above</dd>
                          </div>
                        )}
                      </dl>
                      <p className="mt-3 text-[13.5px] text-ink-2">
                        <span className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">POLARITY JUSTIFICATION: </span>
                        {g.polarityJustification}
                      </p>
                      {g.dataPolicy && (
                        <p className="mt-3 border border-chrome text-[13px] leading-[1.6] text-ink-2 p-2.5">
                          <span className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-stamp">⚠ DATA POLICY: </span>
                          {g.dataPolicy}
                        </p>
                      )}
                      {g.accessType === "api" && g.staleDisclosure && (
                        <p className="mt-3 border border-chrome text-[13px] leading-[1.6] text-ink-2 p-2.5">
                          <span className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-stamp">STALENESS: </span>
                          {resolveStaleDisclosure(g, getGaugeData(g.id))}
                        </p>
                      )}
                      {g.accessType === "api" && !g.staleDisclosure && g.staleAfterMonths !== undefined && (
                        <p className="mt-3 text-[13.5px] text-ink-2">
                          <span className="font-martian-mono text-[9.5px] font-bold tracking-[.12em] text-ink-3">FLAGGED STALE AFTER: </span>
        ...
        <section className="mt-10 pt-8 border-t border-ink">
          ...
          <p className="mt-2 text-ink-2">
            ...
            <strong>live</strong> (fetched by <code className={CODE}>/pipeline</code> or entered by hand from its named
        ...
        <section className="mt-10 pt-8 border-t border-ink">
          ...
          <p className="mt-2 text-ink-2">
            ...
            sub-scores:{" "}
            <Link href="/gauges/cohesion-minority-experience" className={LINK}>
          ...
          <p className="mt-3 text-ink-2">
            ...
            {gaugesConfig.gauges
              ...
              .map((g, i, arr) => (
                <span key={g.id}>
                  <Link href={`/gauges/${g.id}`} className={LINK}>
                    {g.name}
                  </Link>
                  {i < arr.length - 1 ? ", " : ""}
                </span>
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-[13.5px] text-ink-2">
            <li>
          ...
          <p className="mt-2 text-ink-2">
            ...
            2023 National Identity &amp; Citizenship module (GESIS, <code className={CODE}>ZA10010_v1.0.0</code>,
            released 13 March 2026) covers 7 of the {peerCount} peers cleanly — Great Britain was fielded but
            published separately as <code className={CODE}>ZA9132</code> for methodological reasons (GB and Scotland
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-[13.5px] text-ink-2">
            <li>
              <strong>ISSP <code className={CODE}>ZA10010</code> reaching a version with Japan and a usable GB
